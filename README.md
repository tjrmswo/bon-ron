...
- [아키텍처](#아키텍처)
- [기술적 의사결정](#-기술적-의사결정)
  - [1. GPT-4o-mini 채택 — 실측 비교 (vs Embeddings, GPT-5-mini, GPT-5.4-nano)](#1-gpt-4o-mini-채택--실측-비교-vs-embeddings-gpt-5-mini-gpt-54-nano)
  ...
  - [5. 언론사명 파싱 3단계 Fallback](#5-언론사명-파싱-3단계-fallback)
- [실행 방법](#실행-방법)
...
### 1. GPT-4o-mini 채택 — 실측 비교 (vs Embeddings, GPT-5-mini, GPT-5.4-nano)

#### 왜 문제였는가
AI 기반 뉴스 클러스터링의 첫 시도는 `text-embedding-3-small` 임베딩 방식이었습니다. 코사인 유사도 threshold를 0.40까지 낮춰도 "이란 협상 무산"과 "이란 핵 회담 결렬"처럼 같은 사건을 다르게 표현한 한국어 기사는 벡터 거리가 멀어 묶이지 않는 문제가 반복됐습니다. 맥락 이해가 필요하다고 판단해 LLM 기반 클러스터링으로 전환했지만, 이번엔 어떤 모델을 쓸지가 새로운 문제가 됐습니다.
...
### 5. 언론사명 파싱 3단계 Fallback

#### 왜 문제였는가
기사 URL만으로는 언론사명을 안정적으로 알아낼 수 없었습니다. 도메인만으로 추정하면 부정확했고, 일부 언론사는 EUC-KR 인코딩을 사용해 `og:site_name`을 그대로 파싱하면 깨진 문자로 표시되는 문제도 있었습니다.

#### 어떻게 결정했는가
자주 등장하는 주요 언론사는 정적 딕셔너리(`SOURCE_MAP`)로 우선 매칭하고, 딕셔너리에 없는 경우 `og:site_name` 메타 태그를 파싱하도록 fallback을 뒀습니다. EUC-KR 인코딩 언론사는 fetch 응답을 텍스트로 바로 디코딩하지 않고 `arrayBuffer()`로 받은 뒤 `TextDecoder`로 인코딩을 지정해 디코딩하도록 처리했습니다. 그래도 파싱에 실패하면 최종적으로 `tldts`로 도메인만 추출해 표시하도록 3단계 fallback 구조를 만들었습니다.

#### 결과
`SOURCE_MAP → og:site_name 파싱 → 도메인 추출` 순으로 이어지는 3단계 방어 구조로, 등록되지 않은 언론사가 나와도 화면에 빈 값이나 깨진 문자 대신 항상 유의미한 언론사명이 표시되도록 만들었습니다.
