...
[규칙]
1. 본문에 명시된 사실만 사용하세요. 단, when_where는 문맥 추론 가능하면 기입. 추측 금지.
2. 본문에 해당 정보가 없으면 반드시 null을 반환하세요. (when_where 제외)
3. 모든 응답은 한국어로 작성하세요.
...
[출력 스키마]
{
  ...
  "when_where": "60자 이내, 사건 발생 시점·장소만 (발표 장소 제외). 명시되지 않았더라도 문맥에서 추론 가능하면 반드시 기입. 예: 기사에 '지난달 협상'이라고만 써도 추론해서 기입. 정말 알 수 없을 때만 null | null",
